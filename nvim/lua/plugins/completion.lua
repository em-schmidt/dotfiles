...
local function _2_()
  ...
  cmp.setup({experimental = {ghost_text = {hl_group = "Comment"}}, formatting = {fields = {"abbr", "kind", "menu"}, format = lspkind.cmp_format({maxwidth = 50, menu = {copilot = ("[" .. "Copilot" .. "]"), nvim_lsp = ("[" .. "LSP" .. "]"), conjure = ("[" .. "Conjure" .. "]"), buffer = ("[" .. "Buffer" .. "]")}, mode = "symbol", symbol_map = {Copilot = "\239\132\147"}})}, mapping = {["<c-space>"] = cmp.mapping(cmp.mapping.complete({i = "c"})), ["<cr>"] = cmp.mapping.confirm({behavior = cmp.ConfirmBehavior.Insert, select = true}), ["<c-e>"] = cmp.mapping({i = cmp.mapping.abort()}), ["<tab>"] = cmp.mapping(_3_, {"i", "s"}), ["<s-tab>"] = cmp.mapping(_5_, {"i", "s"})}, snippet = {expand = _7_}, sources = {{name = "nvim_lsp"}, {name = "copilot"}, {name = "conjure"}, {name = "buffer"}, {name = "luasnip"}}})
